...
public class Loop implements InstructionHandler 
{
	public InstructionList handle(Operand operand1, Operand operand2, Operand operand3, DynamicInstruction dynamicInstruction) 
	{
		...
		if(operand1.isImmediateOperand() && operand2==null && operand3==null)
		{
			...
			//cx=cx-1
			microOps.appendInstruction(Instruction.getIntALUInstruction(counterRegister,
					Operand.getImmediateOperand(), counterRegister));
			
			//Perform a conditional jump
			ConditionalJump conditionalJump = new ConditionalJump();
			microOps.appendInstruction(conditionalJump.handle(operand1, operand2, operand3, dynamicInstruction));
