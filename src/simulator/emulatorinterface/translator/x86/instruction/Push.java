...
public class Push implements InstructionHandler 
{
	public InstructionList handle(Operand operand1, Operand operand2, Operand operand3, DynamicInstruction dynamicInstruction) 
	{
		...
		Operand stackPointerLocation = Operand.getMemoryOperand(stackPointer, null, dynamicInstruction.getMemoryWriteAddress().firstElement());
		   ...
		   operand2==null &&  operand3==null)
		{
			...
			{
				valueToPush = OperandTranslator.getLocationToStoreValue(operand1, registers);
				
				operand1.setMemoryAddress(dynamicInstruction.getMemoryReadAddress().firstElement());
				microOps.appendInstruction(Instruction.getLoadInstruction(operand1,valueToPush));
			}
			else
			{
			...
			microOps.appendInstruction(Instruction.getStoreInstruction(stackPointerLocation, valueToPush));
		}
