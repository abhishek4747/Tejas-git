...
public class IntegerMultiplication implements InstructionHandler 
{
	public InstructionList handle(Operand operand1, Operand operand2, Operand operand3, DynamicInstruction dynamicInstruction) 
	{
			...
			operand2==null && operand3==null)
		{
			...
			{
				multiplier = OperandTranslator.getLocationToStoreValue(operand1, registers);
				operand1.setMemoryAddress(dynamicInstruction.getMemoryReadAddress().firstElement());
				microOps.appendInstruction(Instruction.getLoadInstruction(operand1,	multiplier));				
			}
			else
			{
				multiplier = operand1;
			...
			microOps.appendInstruction(Instruction.getIntegerMultiplicationInstruction
				...
				operand3==null)
		{
			...
			{
				multiplier = OperandTranslator.getLocationToStoreValue(operand2, registers);
				operand2.setMemoryAddress(dynamicInstruction.getMemoryReadAddress().firstElement());
				microOps.appendInstruction(Instruction.getLoadInstruction(operand2, multiplier));				
			}
			else
			{
				multiplier = operand2;
			 ...
			 operand3.isImmediateOperand())
		{
			...
			if(operand2.isMemoryOperand())
			{
				multiplicand = OperandTranslator.getLocationToStoreValue(operand2, registers);
				operand2.setMemoryAddress(dynamicInstruction.getMemoryReadAddress().firstElement());
				microOps.appendInstruction(Instruction.getLoadInstruction(operand2, multiplicand));				
			}
			else
			{
