...
public class InorderCoreMemorySystem_MII extends CoreMemorySystem {
	...
	public InorderCoreMemorySystem_MII(Core core) {
		...
		containingExecEngine = (MultiIssueInorderExecutionEngine) core
				.getExecEngine();
	}

	// To issue the request directly to L1 cache
	// missPenalty field has been added to accomodate the missPenalty incurred
	// due to TLB miss
	public boolean issueRequestToL1Cache(RequestType requestType, long address) {
		MultiIssueInorderPipeline inorderPipeline = (MultiIssueInorderPipeline) core
				.getPipelineInterface();

		int tlbMissPenalty = performDTLBLookup(address, inorderPipeline);

		AddressCarryingEvent addressEvent = new AddressCarryingEvent(getCore()
				.getEventQueue(), tlbMissPenalty, this, l1Cache, requestType,
				address);
	...
	public void issueRequestToInstrCache(long address) {
		...
		AddressCarryingEvent addressEvent = new AddressCarryingEvent(getCore()
				.getEventQueue(), tlbMissPenalty, this, iCache,
				RequestType.Cache_Read, address);
	...
	private int performITLBLookup(long address,
			MultiIssueInorderPipeline inorderPipeline) {
	...
	private int performDTLBLookup(long address,
			MultiIssueInorderPipeline inorderPipeline) {
	...
	public void handleEvent(EventQueue eventQ, Event event) {
		...
		if (iCache == l1Cache) {
			containingExecEngine.getFetchUnitIn()
					.processCompletionOfMemRequest(address);
			containingExecEngine.getMemUnitIn().processCompletionOfMemRequest(
					address);
		}
		...
		// if response comes from l1Cache, inform memunit
			...
			System.out
					.println("mem response received by inordercoreMemSys from unkown object : "
