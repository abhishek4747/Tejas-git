...
public class FetchUnitIn_MII extends SimulationElement {
	...
	private int fetchBufferIndex; // Index to first instruction to be popped out
									// of fetch buffer
	private boolean fetchBufferStatus[]; // To check whether request to ICache
											// is complete or not

	private boolean sleep; // The boolean to stall the pipeline when a sync
							// request is received
	int syncCount;
	...
	public FetchUnitIn_MII(Core core, EventQueue eventQueue,
		...
		this.fetchBufferCapacity = (int) (core.getIssueWidth() * (SystemConfig.core[core
				.getCore_number()].getICacheLatency()));
	...
	public void fillFetchBuffer(MultiIssueInorderPipeline inorderPipeline) {
		...
		for (int i = (this.fetchBufferIndex + this.fetchFillCount)
				% this.fetchBufferCapacity; this.fetchFillCount < this.fetchBufferCapacity; i = (i + 1)
				% this.fetchBufferCapacity) {

			if (containingExecutionEngine.multiIssueInorderCoreMemorySystem
					.getiCache().isBusy()) {
				break;
			}
			...
			if (newInstruction.getOperationType() == OperationType.inValid) {
				...
				// The first micro-operation of an instruction has a valid CISC
				// IP. All the subsequent
				// micro-ops will have IP = -1(meaning invalid). We must not
				// forward this requests to iCache.
				if (SimulationConfig.detachMemSysInsn
						|| newInstruction.getCISCProgramCounter() == -1) {
					this.fetchBufferStatus[i] = true;
				} else {
					this.fetchBufferStatus[i] = false;
					containingExecutionEngine.multiIssueInorderCoreMemorySystem
							.issueRequestToInstrCache(newInstruction
	...
	public void performFetch(MultiIssueInorderPipeline inorderPipeline) {
		...
		Instruction ins;
		System.out.println("1--> In Fetch unit ");
		...
		while (!this.sleep && this.fetchFillCount > 0
			...
			System.out.println("\tIns: " + ins + " opType:"
			...
			if (ins.getOperationType() == OperationType.sync) {
				...
				if (this.core.TreeBarrier == true) {
					...
					ArchitecturalComponent.coreBroadcastBus.getPort().put(
							new AddressCarryingEvent(0, this.core.eventQueue,
									this.core.barrier_latency,
									ArchitecturalComponent.coreBroadcastBus,
									ArchitecturalComponent.coreBroadcastBus,
									RequestType.TREE_BARRIER, barrierAddress,
									coreId));
				} else {
					if (bar.timeToCross()) {
						sleepThePipeline();
						...
						if (this.core.barrierUnit == 0) {
							if (GlobalClock.getCurrentTime() < bar.time + 35) {
								bar_lat = (int) (this.core.barrier_latency
										+ GlobalClock.getCurrentTime() - bar.time);
							} else
						...
						for (int i = 0; i < bar.getNumThreads(); i++) {
							ArchitecturalComponent.coreBroadcastBus
									.addToResumeCore(bar.getBlockedThreads()
											.elementAt(i));
						}
						ArchitecturalComponent.coreBroadcastBus
								.getPort()
								.put(new AddressCarryingEvent(
		...
		// remove micro-ops from inputToPipeline and issue request to the
		// i-cache
	...
	public void resumePipeline() {
		System.out.println("Resuming the pipeline "
